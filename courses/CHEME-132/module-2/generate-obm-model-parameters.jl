...
# initialize -
df = DataFrame(firm=Int64[], μ=Float64[], σ=Float64[], b=Float64[]);
...
for firm_index ∈ list_of_firm_ids
    ...
    Y = (firm_data[!,:volume_weighted_average_price]);
    ...
    for j ∈ 2:number_of_trading_days
        ...
        growth_rate_array[j-1] = (1/Δt)*((S₂ - S₁)/S₁);
    end

    R = growth_rate_array.*Δt;
    nd = fit_mle(Normal, R);
    σ̂ = params(nd) |> last |> sqrt |> (x -> 100.0*x)
