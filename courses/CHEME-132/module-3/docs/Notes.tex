    ...
    \begin{algorithmic}[1]
		...
		\For{$i\in{1}~\text{to}~{N_{s}}}$\Comment{Loop over the number of samples}
			...
			\For{$j\in{2}~\text{to}~N_{t}$}\Comment{Loop over the number of time steps}
				...
				\EndFor
				\Statex
				\State{$X[i,j,k+1] \gets X[i,j-1,k+1]\cdot\exp\Biggl[\left(\mu[k]-a_{kk}/{2}\right)\Delta{t} + \sqrt{\Delta{t}}\cdot\text{noise}\Biggr]$}
				\EndFor
			\EndFor
		\EndFor
...
% The final share price of each firm in the portfolio $\mathcal{P}$ at time $T$ can be computed using the multi-dimensional GBM model.
% Further, we could sample the fraction of the total investment budget $\omega_{i}$ invested in each firm $i\in\mathcal{P}$ from a
% Dirchlet distribution, which is a multivariate generalization of the beta distribution, and then compute the NPV of the portfolio $\mathcal{P}$ at time $T$. There is a slight disconect here we we need to know the number of shares of each firm in the portfolio $\mathcal{P}$ at time $t$ to compute the NPV of the portfolio $\mathcal{P}$ at time $T$. Then we could calculate the NPV for each sample, and then compute the expected NPV of the portfolio $\mathcal{P}$ at time $T$ by averaging the NPV over all the samples. Alternatively, we could first fit a 
