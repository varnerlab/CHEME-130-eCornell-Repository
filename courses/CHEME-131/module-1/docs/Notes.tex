...
\end{equation}
The product term is the \textit{multi-period discrete discount factor}, which we represent as the function $\mathcal{D}_{t,0}(r)$.
\end{definition}

\subsubsection*{Discounting from a different perspective.}
In the exchange rate model, we convert cash flows from the future to the present (and vice-versa) using a discount rate $r_{t+1,t}$, and a discount factor $\mathcal{D}_{t,0}(r)$.
However, students often find this approach confusing. For example, its not what discount rate to use, what the discount factor means, etc.
Toward this challenge, let's introduce a different perspective on discounting.
Another way to think about discont rates is to consider the discount rate as the minimum rate of return that a decision-maker would accept, 
i.e., as hypothetical interest rate that would be required to make an investment or project worthwhile.
...
\href{https://treasurydirect.gov/marketable-securities/treasury-bills/}{United States Treasury Bills}, or \texttt{T-bills} are Treasury debt instruments with short-term maturity periods T = 4, 8, 13, 26, and 52 weeks 
and zero coupon payments, i.e., no semiannual interest payments (Fig. \ref{fig:t-bill-schematic}).
In the context of the abstract asset framework, T-bills have two cashflow events, namely when the lender (you) buys the T-bill at auction and when the borrower (United States Treasury
pays the lender (you) the face (par) value of the T-bill at maturity.
\begin{figure}[h]
    \centering
	...
	The schematic is written from the bill holder's perspective.}\label{fig:t-bill-schematic}
\end{figure}
